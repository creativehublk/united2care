...
class DBOperations{
    ...
    public function loginAdmin($loginarr){
        ...
        if(password_verify($password,$this->fetch['password'])){
            ...
            $auditTrailsArr = array(
                'user_id' => $user_id,
                'action' => 'Login',
                'description' => $username.' has been logged in',
                'user_agent' => $user_agent,
                'ip_address' => $ip_address
            );
            DBOperations::insertData('audit_trail', $auditTrailsArr);
    ...
    public function checkArtistLogin(){
        ...
        if(isset($_SESSION['artist_id']) && isset($_SESSION['artist_r_pswd']) && isset($_SESSION['artist_name']) ){
            $artist_id = $_SESSION['artist_id'];
            $artist_r_pswd = $_SESSION['artist_r_pswd'];
            $artist_name = $_SESSION['artist_name'];

            // Check login
            $select = mysqli_query($this->localhost, "SELECT * FROM `supplier_logins` WHERE `supplier_id` = '$artist_id' ORDER BY `id` DESC LIMIT 0, 1 ");
            if(mysqli_num_rows($select) > 0){
                $fetch = mysqli_fetch_array($select);
                if($artist_r_pswd == $fetch['r_pswd']){
                    $access = 1;
                }
            }
        ...
        return array('access' => $access, 'name' => $artist_name, 'artist_id' => $artist_id );
    } //checkArtistLogin
    ...
    public function auditTrailsArtist($auditArr){
        ...
        $this->insertData('audit_trail_artist', $insertArr);
    ...
    public function updateData($t,$arr,$where = null){
        ...
        $this->query = "UPDATE ".$this->table." SET " . join(', ', $this->data) . " WHERE ". join(' AND ', $this->where) ." ";
    ...
    public function delete($t,$whereArr){
    ...
    public function numbRows($t,$whereArr){
        ...
        $this->query = "SELECT `id` FROM ".$this->table." WHERE ". join(' AND ', $this->where) ." ";

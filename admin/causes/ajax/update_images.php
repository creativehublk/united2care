...
if($checkAccess == 1){
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if(isset($_POST['update_product_id'])){
            ...
            $cause_id = $_POST['update_product_id'];

            $image_exist_id_arr = $_POST['image_id_if_exist'];
            $images_arr = $_FILES['gallery-img'];
            
            $del_query = "DELETE FROM `causes_images` WHERE `id` NOT IN (".join(', ', $image_exist_id_arr).") AND `cause_id`= '$cause_id' ";
                ...
                for($i=0; $i < $count; $i++ ){
                    ...
                    if(isset($images_arr['name'][$i]) ) {
                        ...
                        if(strlen($images_arr['name'][$i]) > 0 ) {
                            ...
                            $path = ROOT_PATH."/admin/uploads/causes/";
                            if (!file_exists($path)) {
                            ...
                            if($pass_parm['error'] == 0){
                                ...
                                $update = mysqli_query($localhost, "INSERT INTO `causes_images` (`cause_id`,`name`) VALUES('$cause_id', '$file_to_save') ");
